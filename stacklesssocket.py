...
for k, v in stdsocket.__dict__.iteritems():
    if k.upper() == k or k in ['htonl', 'htons', 'ntohl', 'ntohs']:
        globals()[k] = v
...
class dispatcher(asyncore.dispatcher):
    ...
    def __init__(self, sock):
        ...
        # if self.socket.getsockopt(SOL_SOCKET,SO_TYPE) == SOCK_DGRAM:
    ...
    def writable(self):
        if self.socket.getsockopt(SOL_SOCKET,SO_TYPE) != SOCK_DGRAM and not self.connected:
            return True
